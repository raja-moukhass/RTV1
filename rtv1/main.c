...
int    intersection_spher(t_ray *r,t_sphere *s)
{
...
int		 intersection_cylinder(t_data *data, t_vec *p, t_vec *n)
{
    ...
    t_vec delta_p = vec_sub(data->ray.o,data->cylinder->pos);
    // delta_p = normalize(delta_p);
	double tmp = dot_product(data->ray.dir,data->cylinder->axis);
	double a = dot_product(data->ray.dir, data->ray.dir) - pow(tmp, 2);
    
    double tmp1 = dot_product(delta_p, data->cylinder->axis);
    double b = 2 * (dot_product(data->ray.dir, delta_p) - (tmp * tmp1));
    double c = dot_product(delta_p, delta_p) - pow(tmp1,2) - (data->cylinder->angle*data->cylinder->angle);
...
void    ray_tracer(t_data *data)
{
	...
	while (y < HEIGHT)
	{
		...
		while (x++ < WIDTH)
        {
            ...
            // if (intersection_cone(data, &p, &(data->var.n))){
...
//             t = 0;
        ...
        //     p2 = 0;
            ...
            if (intersection_cylinder(data, &p, &(data->var.n))){
